...
def get_densuke_data(url):
    ...
    # テスト用に今後14日間を探すようにします
    target_dates = [today + datetime.timedelta(days=i) for i in range(14)]
    print(f"検索対象の日付リスト: {[d.strftime('%m/%d') for d in target_dates]}")
    ...
    if not found_event:
        print("❌ 伝助の中に該当する日付の予定が見つかりませんでした。")
    ...
    if target_header in description:
        ...
        print(f"⚠️ {target_header} という見出しが備考欄にありませんでした。")
        gaso_plan = "備考欄に予定が記載されていません。"
...
def send_to_line(message):
    ...
    print(f"--- LINE送信開始 ---")
    print(f"送信先ID: {group_id[:5]}***") # セキュリティのため一部隠して表示
    ...
    print(f"LINE応答内容: {response.text}")
...
if event_text:
    ...
    if m:
        ...
        print(f"❌ 予定名 '{event_text}' の中に [四, ひ, ラ] のいずれも見つかりませんでした。")
else:
    print("❌ 送信対象のイベントがないため、LINE送信をスキップしました。")
